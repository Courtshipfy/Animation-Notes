...
| Kai | 四向平移起步 + 可选 90°/180°旋转起步 | 独立起步段；`FirstStepAlpha` 把动画时间 `[0.1, 0.4]` 映射为「第一步是否已建立」，距离匹配按累计位移约 `1` 定位第一步相位 |
| Motion Matching | 无独立起步资产 | 起步是任意连续 clip 里「速度 0 → 加速」的片段，由轨迹匹配选中，步数不固定 |

Kai 的 `FirstStepAlpha` 和「距离 1」定位最能说明离散方案的语义：起步 clip 的锚点就是**第一步落地**。ALS 的 `Acceleration` 同理，是覆盖起步前 1~2 步的短段。Motion Matching 则根本不要求存在「一段叫起步的动画」——它可能匹配到某个 10 米直线行走 clip 的前 3 步，也可能匹配到某个变向 clip 的加速段。
